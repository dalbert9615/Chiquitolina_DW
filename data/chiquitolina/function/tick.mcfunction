...
# Temporizador actionbar
# execute as @a[scores={chiquitolina_timer=0..}] run scoreboard players add @s timer_cycle 1
# execute as @a[scores={timer_cycle=1200..}] run scoreboard players remove @s minutos 1
# execute as @a[scores={timer_cycle=1200..}] run scoreboard players set @s timer_cycle 0
# execute as @a[scores={chiquitolina_timer=0..3599}] run title @s actionbar ["Chiquitolina: ",{"score":{"name":"@s","objective":"minutos"}},{"text":" min"}]
